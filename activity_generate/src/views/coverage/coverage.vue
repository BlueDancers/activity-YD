 ...
 * @LastEditTime: 2020-05-10 22:11:06
  ...
  <div class="coverage">
    ...
    <div class="list">
      <cell
        ...
        :key="'cell'+index"
        @click.native="addItem(item.activityId)"
        :title="item.name"
        :class="{'cells':1==1 ,'activeCells':isActive(item.activityId)  }"
        draggable="true"
...
export default {
  ...
  methods: {
    ...
    setZIndex(id, index) {
      for (
        let i = 0, len = this.$store.state.core.template.length;
        i < len;
        i++
      ) {
  ...
  computed: {
    templateLists() {
      ...
      tmp.map(item => {
        if (!this.cache.keyValue.namekey.includes(item.activityId)) {
          //没存储的对象

          if (!this.cache.nameCache.namekey.includes(item.name)) {
            //新的类型
          ...
          this.cache.keyValue.value[
            this.cache.keyValue.namekey.indexOf(item.activityId)
          ] = this.cache.nameCache.value[
            this.cache.nameCache.namekey.indexOf(item.name)
          ]++;
        } else {
        }
        item.name += this.cache.keyValue.value[
          this.cache.keyValue.namekey.indexOf(item.activityId)
        ];
      });
      tmp.reverse().sort((a, b) => {
